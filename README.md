...
> **由 LLM 直接驱动的可交互演示生成器**：自然语言一句话生成可点击、有动效、能发布的演示网站，由结构化 DSL（不是直出代码）保证可控、可编辑、稳定。
...
演示是「视觉代码」——LLM 擅长写代码，但市面上工具要么走纯静态 deck（Gamma / Tome）失去交互能力，要么走直出 React（v0 / bolt.new）失去结构与可控。华胥说选择第三条路：**LLM 输出 DSL，运行时确定性渲染**。
